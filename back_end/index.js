...
*/
//console.info(new Date() , ' - DB_POOL_CONNECTION_LIMIT=',DB_POOL_CONNECTION_LIMIT);
...
//console.info(new Date() , ' - db.config.connectionLimit=',db.config.connectionLimit);

// evento disparado quando o programa faz uma nova conexao com o bd (dentro do pool)
db.on('connection', function (connection) {
  console.info(new Date() , ' - MySQL Connection '+ connection.threadId +' established');
  console.info(new Date() , ' - MySQL Connections in the pool. Total=',db._allConnections.length);
  console.info(new Date() , ' - MySQL Connections in the pool. Free=',db._freeConnections.length);

  connection.on('error', function (err) {
    console.error(new Date(), ' - MySQL error event on connection ' + connection.threadId, ' - ' , err.code);
  });

  // nao vi ser executado e acho que eh porque o close das conexoes do pool nao eh executado
  connection.on('close', function (err) {
    console.error(new Date(), ' - MySQL close on connection ' + connection.threadId, err);
  });

  connection.on('end', function () {
    console.info(new Date(), ' - MySQL end event on connection ' + connection.threadId );
  });
...
db.on('acquire', function (connection) {
  console.log(new Date() , (' - MySQL Connection '+ connection.threadId +' acquired from the pool'));
  console.info(new Date() , ' - MySQL Connections in the pool. Total=',db._allConnections.length);
  console.info(new Date() , ' - MySQL Connections in the pool. Free=',db._freeConnections.length);
});
...
db.on('release', function (connection) {
  console.log(new Date() , ' - MySQL Connection ' + connection.threadId + ' released');
  console.info(new Date() , ' - MySQL Connections in the pool. Total=',db._allConnections.length);
  console.info(new Date() , ' - MySQL Connections in the pool. Free=',db._freeConnections.length);
});
